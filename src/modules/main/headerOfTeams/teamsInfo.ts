import {PRESS_NEXT, PRESS_PREV} from "../../../storages/constants";
...
export type TTeamIds = [number, number];
...
const generateRandomTeams = (teams?: TTeamIds): TTeamIds => {
  let first = Math.ceil(Math.random() * 14);
  let second = Math.ceil(Math.random() * 14);
  while (first === second) {
    second = Math.ceil(Math.random() * 14)
  }

  if (teams) {
    let index = Math.round(Math.random());
    if (index) {
      while (teams[1] === second) {
        second = Math.ceil(Math.random() * 14);
      }
    } else {
      while (teams[0] === first) {
        first = Math.ceil(Math.random() * 14);
      }
    }
  }
...
const history: Array<TTeamIds> = [generateRandomTeams()];
...
const teamsInfo = (state: ITeamsState = initialState, action: any) => {
  switch (action.type) {
    case PRESS_PREV:
      ...
      history.pop();
      sessionStorage.removeItem(`${history.length - 1}`);
      return {
        ...state, ...{
    ...
    case PRESS_NEXT:
      let randomTeams = generateRandomTeams();

      while (history[history.length - 1][0] === randomTeams[0] || history[history.length - 1][1] === randomTeams[1]) {
        randomTeams = generateRandomTeams(history[history.length - 1]);
      }

      history.push(randomTeams);
